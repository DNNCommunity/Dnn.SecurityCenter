...
export class LocalizationClient extends ClientBase {
     ...
     * @return OK
     */
    getLocalization(signal?: AbortSignal | undefined): Promise<LocalizationViewModel | null> {
...
export class SecurityClient extends ClientBase {
     ...
     */
    getSecurityBulletins(versionString: string | null, signal?: AbortSignal | undefined): Promise<SecurityBulletinsViewModel | null> {
    ...
    protected processGetSecurityBulletins(response: Response): Promise<SecurityBulletinsViewModel | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = resultData200 ? SecurityBulletinsViewModel.fromJS(resultData200) : <any>null;
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<SecurityBulletinsViewModel | null>(null as any);
    }
}
